...
const GradeEntryForm = ({ 
  ...
  return (
    ...
    <Card className="p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Grade Entry
                    </h3>
    ...
    {/* Grade Entry Table */}
    {selectedClass && selectedAssignment && <Card className="overflow-hidden">
        <div
            className="p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100">
            <h4 className="text-lg font-semibold text-gray-900">Student Grades - {assignments.find(a => a.id === selectedAssignment)?.name}
            </h4>
        </div>
        <div className="overflow-x-auto">
            <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th
                            className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase">Student
                                              </th>
                        <th
                            className="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase">Score
                                              </th>
                        <th
                            className="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase">Percentage
                                              </th>
                        <th
                            className="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase">Letter Grade
                                              </th>
                    </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                    {students.filter(student => student.status_c === "Active").map(student => {
                        const grade = grades.find(g => g.studentId === student.Id && g.assignment === selectedAssignment);
...
<td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4 text-center">
                                    {score ? <Badge variant={getGradeColor(percentage)}>
                                        {calculateLetterGrade(percentage)}
                                    </Badge> : <span className="text-gray-400 text-sm">-</span>}
                                </td>
                            </tr>
                        );
                    })}
